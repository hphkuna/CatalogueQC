...
function QC_01_04_XLineVsDepth(MeasCat, Rotation, varargin)
...
MaxDist = 5; % maximum distance to event in km
...
plot(XLineDistance, EventDepth, '.', 'MarkerSize', 4, 'Color', FirstColor)
...
if ~isempty(PlotLimits)
    ...
    xlim([min(XLineDistance) max(XLineDistance)])
    ylim([0 max(EventDepth)])
end

xlabel('X-line Distance [km]')
ylabel('Hypocenter Depth [km]')
title('X-line distance vs. depth', 'FontSize', 16, 'FontWeight', 'bold')

print(gcf,'CurrentFigures/QC_01_04_XLineVsDepth','-dpng', '-r300')

%% density of events

[Density, X, Y] = xDensityPlot(XLineDistance, EventDepth, GridStep, MaxDist); 
...
ylabel('X-line Distance [km]')

print(gcf,'CurrentFigures/QC_01_04_XLineVsDepthDensity','-dpng', '-r300')
